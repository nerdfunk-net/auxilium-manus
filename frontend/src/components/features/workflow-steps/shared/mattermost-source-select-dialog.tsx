...
interface MattermostSourceSelectDialogProps {
  open: boolean;
...
export function MattermostSourceSelectDialog({
  open,
  selectedSourceId,
  onClose,
  onSave,
}: MattermostSourceSelectDialogProps) {
  const router = useRouter();
  const [sourceId, setSourceId] = useState(selectedSourceId);
  const [prevOpen, setPrevOpen] = useState(open);
  const { data, isLoading } = useMattermostSourcesQuery();
  ...
  return (
    <Dialog open={open} onOpenChange={(isOpen) => !isOpen && onClose()}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Mattermost source</DialogTitle>
          <DialogDescription>
            A configured Mattermost source is required. Choose which saved
            source (from Settings → Sources) this step should use. Only the
            source ID is stored on the step; the URL and token are resolved
        ...
        <div className="space-y-4 py-2">
          {isLoading ? (
            ...
            <div className="rounded-lg border border-dashed bg-muted/40 px-4 py-3 text-sm text-muted-foreground">
              <p>No Mattermost sources configured yet.</p>
              <Button
                className="mt-3 h-8"
                size="sm"
                type="button"

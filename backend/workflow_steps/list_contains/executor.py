...
def _parse_list_contains_config(
    config: dict[str, Any],
    ...
    return list_path, field, value_expr, case_sensitive


def _enrich_membership_result(
...
def _classify_device_membership(
    *,
    device: DeviceContext,
    device_id: str,
    node_id: str,
    ...
    raw_list = resolve_device_value(device, list_path, run_id=run_id)
    ...
    enriched = _enrich_membership_result(
        device,
        node_id=node_id,
        matched=matched,
        matched_item=matched_item,
    ...
    return ("match" if matched else "mismatch"), enriched


def _build_list_contains_outcomes(
    *,

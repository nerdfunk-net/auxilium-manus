...
async def _fetch_one_device(
    *,
    ...
    logger.info(
        "%s calling shim device=%s host=%s os=%s source=%s features=%s",
        _STEP_ID,
        device_id,
        bag.get("host"),
        bag.get("os"),
    ...
    if not result or not result.get("success", False):
        ...
        logger.warning(
            "%s device connect/learn failed device=%s host=%s error=%s",
            _STEP_ID,
            device_id,
            bag.get("host"),
            message,
    ...
    if all(not r["success"] for r in feature_results.values()):
        ...
        logger.warning(
            "%s every feature failed device=%s host=%s error=%s",
            _STEP_ID,
            device_id,
            bag.get("host"),
            message,

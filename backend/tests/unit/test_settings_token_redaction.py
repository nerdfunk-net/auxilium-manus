...
class TestSettingsTokenRedaction:
    def test_list_redacts_nautobot_and_git_tokens(self) -> None:
        ...
        assert nautobot.value["token"] == "nb-secret"
        assert git.value["token"] == "git-secret"
    ...
    def test_get_redacts_token_and_sets_token_configured(self) -> None:
        ...
        assert stored.value["token"] == "nb-secret"
    ...
    def test_create_git_source_persists_token_and_redacts_response(self) -> None:
        service = _service()
        persisted: dict = {}
        ...
        assert persisted["value"]["token"] == "git-secret"
    ...
    def test_create_nautobot_source_persists_token_without_dns(
        ...
        response = service.create_setting(
            SettingCreate(key="sources.nautobot.lab", value=dict(NAUTOBOT_VALUE)),
        )

        assert response.value["token"] == ""
        assert response.value["token_configured"] is True
        assert persisted["value"]["token"] == "nb-secret"

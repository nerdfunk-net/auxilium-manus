...
def test_resolve_credentials_scopes_to_global_only() -> None:
    ...
    mock_svc.get_decrypted_password.return_value = "shh"

    with (
        patch("services.credentials.manager.CredentialsService", return_value=mock_svc),
    ...
    mock_svc.list_credentials.assert_called_once_with(
        include_expired=False, acting_user_id=None
    )
    mock_svc.get_decrypted_password.assert_called_once_with(1, acting_user_id=None)
    assert username == "git"
    assert token == "shh"

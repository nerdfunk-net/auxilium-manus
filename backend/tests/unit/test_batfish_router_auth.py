"""FastAPI TestClient tests for auth/permission gating on Batfish source routers."""
...
def _build_app() -> FastAPI:
    app = FastAPI()
    app.include_router(batfish_source_crud_router, prefix="/api")
    app.include_router(batfish_source_ops_router, prefix="/api")
    return app
...
def test_list_sources_requires_auth(app: FastAPI) -> None:
    with TestClient(app) as client:
        response = client.get("/api/sources/batfish")
...
def test_create_source_requires_write_permission(
    ...
    with TestClient(app) as client:
        response = client.post(
            "/api/sources/batfish",
            json={"source_id": "lab", "host": "batfish"},
        )

    assert response.status_code == 403
    assert "sources.batfish:write" in response.json()["detail"]
...
def test_delete_source_requires_delete_permission(
    ...
    with TestClient(app) as client:
        response = client.delete("/api/sources/batfish/lab")

    assert response.status_code == 403
    assert "sources.batfish:delete" in response.json()["detail"]


def test_test_connection_requires_auth(app: FastAPI) -> None:
    with TestClient(app) as client:
        response = client.post(
            "/api/sources/batfish/test-connection", json={"source_id": "lab"}
        )
    assert response.status_code == 401


def test_test_connection_success_reports_network_count(
    app: FastAPI, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setattr(RBACService, "has_permission", lambda self, *_a, **_k: True)
    ...
    async def _check_health(*_a, **_k):
        return ["manus-workflow-1", "manus-workflow-2"]

    mock_batfish.check_health = _check_health
    ...
    assert "2 network(s)" in body["message"]


def test_test_connection_reports_coordinator_unreachable(
    app: FastAPI, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setattr(RBACService, "has_permission", lambda self, *_a, **_k: True)
    ...
    async def _check_health(*_a, **_k):
        raise BatfishAPIError("connection refused")

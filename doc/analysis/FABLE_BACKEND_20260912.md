...
**Update 2026-09-12 (post-fix):** all seven blockers above are implemented and tested — see
`doc/plans/FABLE_20260912.md` and the Status column in §3.2/§4.2/§5.2. Commits `3e38e0e` (V1) and
`406b827` (R1, R2, V2–V4, T1). None of V5–V14, T2–T6, R3–R8, or the W-series findings were in scope
for this pass and remain open as documented.
...
| Periodic tokens with renew-self | ✅ renew cadence now follows the granted lease, with startup mismatch warning (V4 fixed) |
| Token never persisted | ✅ |
| TLS verification on | ✅ `VAULT_VERIFY_SSL=false` refused outside development (V1 fixed) |
...
| KV v2 versioning handled deliberately (destroy / max_versions) | ✅ delete destroys all versions, update destroys the superseded one (V3 fixed) |
...
**Blockers — all seven fixed 2026-09-12, see `doc/plans/FABLE_20260912.md`**
(commits `3e38e0e` V1, `406b827` R1/R2/V2–V4/T1; full test suite, ruff, pyright,
pip-audit, and the four guard scripts all green)

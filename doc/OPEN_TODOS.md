...
## No lock protects concurrent writers to the same git working tree

**Added:** 2026-09-21 · **Area:** `backend/services/git`, `backend/services/execution/step_runner`, `doc/WORKFLOW-STEPS.md`

### What we have

`services/git/service.py::GitService` (and `GitRepositoryService`/
`load_git_repository`) opens one shared on-disk working tree per
`GitRepository` row (`load_git_repository` → single `path`), with **no
locking anywhere in the git service layer** — checked `services/git/service.py`,
`services/git/repository_service.py`, and
`workflow_steps/common/git_repository_loader.py` directly. `store-artifact`
(destination: git) and `git-clone`/`git-pull`/`git-push` are documented as
"not concurrency-safe" for exactly this reason (`doc/WORKFLOW-STEPS.md` →
"Writing concurrency-safe steps"), but the engine does nothing to enforce or
prevent the unsafe case for them — it's pure author-vigilance.

`open-change-request` is the one exception: `repo_stage_lock`
(`services/change_requests/repo_lock.py`, Redis `SET NX EX`, fail-soft if
Redis is down) already serialises concurrent callers against the same repo —
across runs, fan-out children, *and* sibling branches — so it doesn't
corrupt the working tree. It still produces a separate branch/commit/change-
request row per caller though, so placement after a join point still matters
for correctness, just not for data integrity. The other four steps below
have no equivalent lock at all.

This was always true for fan-out children (which may run on different
Hatchet workers — an in-process lock couldn't help there anyway). It's now
also true for two independent, non-fan-out branches within the *same* run:
as of branch-level parallelism (`StepRunner._run_wave`,
`services/execution/graph.py::topological_generations`), sibling branches
with no dependency edge between them run concurrently in the same process.
Two such branches both targeting the same `GitRepository` (no Fan In /
explicit dependency forcing them apart) will race on `index.lock`, produce N
single-file commits instead of one, or reject a non-fast-forward push — same
failure mode as the fan-out case, just via a second trigger.

### Original goal

Either prevent the race outright for the half that's actually fixable
(same-process sibling branches — an `asyncio.Lock` keyed by
`git_repository_id` on `StepRunner`, mirroring `DeviceSessionPool`'s
per-device lock, would fully close that half), or at minimum make the hazard
visible at author-time (save-time canvas validation, or a lint) instead of
relying on someone having read the doc.

### Why it's deferred

Raised and discussed while shipping branch-level parallel execution
(`feature/parallel-steps`, 2026-09-21) — deliberately scoped out of that work
as doc-only for now (see `doc/WORKFLOW-STEPS.md` → "Writing concurrency-safe
steps"), since an in-process lock only protects half the hazard (fan-out
children can be cross-process) and a partial fix that silently "solves" the
common case risked masking that the fan-out half still isn't protected by
anything. No concrete incident yet — needs a real workflow that actually
hits this before deciding fix vs. mitigate vs. leave-as-author-responsibility.

### When we revisit

If a real workflow surfaces this (git push failures / duplicate commits from
a run with independent branches or wide fan-out hitting the same repo):

1. For the same-process (sibling-branch) half: add
   `self._git_repo_locks: dict[int, asyncio.Lock]` to `StepRunner`, acquired
   by `git_repository_loader.load_git_repository` (or by each git-touching
   executor) before touching the working tree.
2. For the cross-process (fan-out) half: no in-process lock can help;
   `repo_stage_lock` (`services/change_requests/repo_lock.py`) is a ready-made
   pattern to extend to the other four steps — a Redis `SET NX EX` lock keyed
   by `git_repository_id`, fail-soft if Redis is down. Alternatively, a
   save-time canvas validation that flags a git-touching step reachable from
   a fan-out inventory step without an intervening Fan In node.
3. Either way, update `doc/WORKFLOW-STEPS.md` → "Writing concurrency-safe
   steps" once enforcement exists so the doc describes a guarantee instead of
   a rule the author must remember.
